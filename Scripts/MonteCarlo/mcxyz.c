...
int main(int argc, const char * argv[]) {
	...
	char 	*v=NULL; //intitalize the pointer (address location, it is equal to NULL because you don't know what data location)
	float 	*F=NULL;
	float	*R=NULL;
	int 	type;
	NN = Nx*Ny*Nz; //Type int (Number of x bins)
	//Malloc allocates a chunk of memory and returns a pointer to the start of that memory location
	v  = ( char *)malloc(NN*sizeof(char));  /* tissue structure */ //allocates memory for each tissue type character and lets computer know anything here is a character
